...
반영된 뒤, 변경된 `*_series.json`과 함께 같은 커밋으로 기본 브랜치에 직접 push된다. 다음
실행은 그 push가 완료된 직후부터 갱신된 `cutoff`를 읽는다(FR-015). 병합 전 검토 단계는
두지 않으며, 잘못 반영된 경우 관리자가 `git revert`로 되돌린다.
