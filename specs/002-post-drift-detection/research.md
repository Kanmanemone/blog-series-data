...
필드가 없는 레코드는 "변경 여부 불명"으로 간주해(spec.md Assumptions) 재확인 후보에
포함시킨다 — 즉 `!record.lastMod || sitemapPost.lastmod > new Date(record.lastMod)`를
후보 판정 조건으로 쓰면, 필드가 없는 레코드는 조건의 앞쪽 절에서 항상 참이 된다.
재확인이 끝나면 그 레코드는 `title`·`lastMod`가 채워져 이후 정상적인 diff 대상이 된다.

**수정(구현 검증 중 발견, `/speckit-converge` T022)**: 이 절이 처음 쓰였을 때는
"자연히 첫 실행에서 한 번씩 재확인된다"고 서술했으나, 이는 실제로 331건 **전부가
동일한 한 번의 실행에** 몰린다는 뜻이라 바로 아래 "일괄 마이그레이션 기각" 근거와
스스로 모순됐다. `node --test`를 실수로 `index.js`에 직접 실행시켜 실제
kenel.tistory.com에 스로틀링 없는 요청이 나가는 사고로 이 모순이 실측됐다(tasks.md
T018 참고). 이후 `selectDriftCandidates`(scripts/sync-tistory-series/index.js)에
`lastMod` 없는 레코드 전용 회당 처리 상한(`MAX_UNKNOWN_LASTMOD_REFETCH_PER_RUN = 30`)을
추가해, 실제로 여러 번의 정기 실행(001의 6시간 주기 기준 약 11~12회, 약 3일)에 걸쳐
나눠 처리하도록 고쳤다. 상한 값 자체는 331건을 감당 가능한 기간(며칠) 안에 소진하면서
한 번에 나가는 요청 수를 두 자릿수로 억제하는 선에서 고른 것으로, 정밀한 튜닝이
필요한 값은 아니다(spec.md가 정확한 배치 크기를 요구하지 않으므로 임의로 확정 짓지
않고, 필요하면 나중에 상수 하나만 조정하면 됨).
...
동일한 스타일) — 다만 "점진적"이 실제로 성립하려면 위의 회당 상한이 반드시 필요하다.
...
**Known limitation, 같은 실행 내는 해결됨 (`/speckit-converge` F1)**: 최초 구현은 재분류
후보를 한 게시글씩 순서대로 판단했다. 재분류 대상 게시글 A와, A가 합류해야 새 시리즈의
2개 임계값을 채워줄 게시글 B가 **같은 실행에서** 함께 감지돼도, A를 먼저 처리할 때는
대상 그룹이 아직 1명(A 자신)뿐이라 이동을 보류하고, 이어서 B를 처리할 때도 "새 seriesId
그룹에 A가 이미 있는가"를 확인하면(A는 여전히 옛 위치에 남아있으므로) B 혼자로도 여전히
1명으로 보여 둘 다 보류되는 버그가 있었다 — 편집이 한 세션에 몰려 같은 lastMod 구간에
잡히는 흔한 경우조차 실패했다는 뜻이라, 애초에 이 문서가 "흔치 않은 경우"로 분류했던
전제 자체가 틀렸다. 구현 검증 중 이를 발견해 `resolveReclassifyBatches`로 수정했다 —
재분류 후보를 목표 seriesId별로 먼저 모두 모은 뒤, 배치 전체 합산으로 2개 이상인지 한
번에 판단하고 실행한다. 이제 같은 실행에서 함께 감지된 A·B는 정상적으로 함께 이동한다.

**남은 잔여 리스크(해결 안 함)**: A와 B가 서로 다른 실행에서 각각 감지되는 경우는 여전히
해결하지 않는다 — 배치 판단은 "같은 실행에서 함께 감지된 후보들"에 대해서만 이뤄지므로,
A가 먼저 감지된 실행에서 이동이 보류된 뒤, B가 나중 실행에서 감지되면 그 시점에 A는 이미
과거 실행에서 보류 처리되어 배치 결정에 그대로 남아있는 상태이고, B의 배치(원소 1개)만
놓고 판단하면 여전히 1명뿐이라 보류된다. 완전한 해결(예: 배치 결정 전체를 매 실행마다
재계산해 보류된 A도 함께 재평가)은 FR-006의 "부분 갱신" 요구와 충돌하므로 채택하지
않는다. 이는 spec.md Assumptions가 이미 "대량 재발행 등 처리량 급증 시나리오는 이
스펙에서 확정하지 않고 실제로 관찰되면 재검토한다"고 여지를 둔 범위와 같은 종류의
잔여 리스크로 취급한다.
